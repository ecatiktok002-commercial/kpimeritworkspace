...
export default function AddTaskModal({ isOpen, onClose, onSubmit, staffList = [] }: AddTaskModalProps) {
  ...
  const handleSubmit = () => {
    ...
    onSubmit(title, '', mins, status, status === 'running' ? getKLTime() : commencementDate, collabNames, workflow, collabIds, frequency);
    // Reset
    setTitle('');
    setMins(120);
    setStatus('queued');
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-8 pb-2 space-y-5 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-[10px] font-bold uppercase tracking-widest block mb-2 text-on-surface-variant ml-1">Pipeline</label>
              <div className="relative">
                <select
                  value={status}
          ...
          <div className={status === 'running' ? 'opacity-50 pointer-events-none' : ''}>
            <label className="text-[10px] font-bold uppercase tracking-widest block mb-2 text-on-surface-variant ml-1">
              {status === 'running' ? 'Commencement Time (Synced)' : 'Commencement Date & Time'} <span className="text-error">*</span>
            </label>
            <div className="relative">
              <input
                type="datetime-local"
                ...
                className="w-full bg-surface-container rounded-2xl py-4 px-5 outline-none border border-outline-variant/5 text-on-surface font-bold focus:border-primary/30 focus:ring-4 focus:ring-primary/5 transition-all shadow-inner pl-12"
              />
            ...
            {status === 'running' && <p className="text-[9px] text-primary font-bold mt-1.5 uppercase tracking-widest px-1">Auto-synced · Kuala Lumpur GMT+8</p>}
          </div>
