...
export default function MeritKPIApp() {
    ...
    if (isLoggedIn && authProfile) {
      const fetchData = async () => {
        try {
          ...
          const { data: taskData, error: taskError } = await query.order('commencement_date', { ascending: true });
          if (taskData) {
            setTasks(taskData.map(t => ({
              ...
              tierVal: t.tier_val,
              points: t.points,
              commencementDate: t.commencement_date,
              ownerId: t.owner_id,
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTasks(prev => prev.map(t => {
        ...
        if (!t.isContinuous && t.elapsedSec >= t.totalSec) return { ...t, status: 'completed' as const };
  ...
  const mapTaskToDB = (task: Task) => ({
    ...
    owner_id: task.ownerId || authProfile?.id || 'unknown',
  ...
  const handleAddTask = useCallback((title: string, note: string, mins: number, status: 'queued' | 'running', commencementDate: string, collaborators: string[] = [], workflow: { id: string; name: string; isCompleted: boolean }[] = [], collaboratorIds: string[] = [], frequency: TaskFrequency = { type: 'once' }, isContinuous: boolean = false) => {
    ...
    const task: Task = {
      ...
      ownerId: isUUID(currentUserId || '') ? currentUserId! : (authProfile?.access_id || 'local'),
      collaborators,
    ...
    // PERSIST TO SUPABASE
    supabase.from('tasks').insert([mapTaskToDB(task)]).then(({ error }) => {
      if (error) {
        console.error('[tasks] save error:', error);
        alert('Warning: Task could not be saved to cloud. It may be lost on refresh.');
      }
    });
  ...
  const completeTask = async (id: string) => {
    ...
    // 1. Create activity log entry for points
    ...
    if (actError) console.error('[activity_log] save error:', actError);

    // 1.5 Check for achievement unlocks
    ...
    if (newlyUnlocked.length > 0) {
      ...
      // Log achievements in activity log
    ...
    // 2. Update staff total points globally
    if (staffId) {
      // Update local team state for manager dashboard chart
    ...
    // 3. Update local task state and persist
    setTasks(prev => prev.map(t => {
      if (t.id === id) {
        ...
        if (isRecurring) {
          ...
          updatedTask = { ...t, status: 'completed' as const, elapsedSec: t.totalSec, lastCompletedDate: getKLTime() };
        }

        // Save updated task to Supabase
        supabase.from('tasks').upsert([mapTaskToDB(updatedTask)]).then(({ error }) => {
  ...
  const addWorkflowStep = (taskId: string) => {
    ...
    setTasks(prev => prev.map(t => {
      if (t.id === taskId) {
        ...
        return { ...t, workflow: [...(t.workflow || []), newStep] };
      }
      return t;
    }));
  ...
  const handleAddModule = useCallback(async (code: string, title: string, desc: string, points: number) => {
    const newMod = { id: crypto.randomUUID(), code, title, description: desc, meritValue: points, participants: 0 };
    
    const { error } = await supabase.from('skill_modules').insert([{
      id: newMod.id,
      code: newMod.code,
      title: newMod.title,
  ...
  const handleUpdateProfile = async (newProfile: StaffProfile) => {
    ...
    if (isLoggedIn && authProfile) {
      ...
      if (!authProfile.id || !isUUID(authProfile.id)) {
        console.warn('Skipping database sync: Invalid Profile UUID');
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if(aid === 'ecaworkspace' && pass === '123456') {
      ...
      if (mData) {
        ...
        setAuthProfile({ id: aid, is_manager: true, role: 'Manager', full_name: 'ECA Workspace', access_id: aid });
      }
          ...
          <section className="mb-12">
            <div className="merit-card-pattern rounded-[40px] p-10 text-white shadow-2xl relative overflow-hidden flex flex-col items-center text-center">
              <div className="relative z-10 w-full">
                ...
                <p className="text-[11px] uppercase font-black tracking-[0.3em] opacity-60 mb-3">Current Month ({new Intl.DateTimeFormat('en-US', { month: 'long', year: 'numeric' }).format(new Date())})</p>
                      ...
                      <div className={`rounded-3xl p-6 border transition-all duration-300 shadow-sm overflow-hidden relative ${
                        ...
                        <div className={`flex justify-between items-start gap-4 mb-2 ${editingWorkflowTaskId === task.id ? 'opacity-30 pointer-events-none' : ''}`}>
                          ...
                          <div className="flex gap-1.5 shrink-0">
                            ...
                            <button onClick={() => handleDeleteTask(task.id, task.title)} className="text-on-surface-variant hover:text-error transition-colors bg-surface-container-high rounded-full w-8 h-8 flex items-center justify-center shrink-0">
                        ...
                        {isActive && (
                           <div className="bg-white rounded-2xl p-4 border border-primary/10 mb-4 shadow-sm">
                               ...
                               <div className="w-full h-2 bg-surface-container rounded-full overflow-hidden">
                                 <div className={`h-full rounded-full transition-all duration-1000 striped-progress ${task.isContinuous ? 'bg-secondary w-full opacity-60' : 'bg-primary'}`} style={!task.isContinuous ? { width: `${pct}%` } : undefined} />
                               </div>
                            </div>
                        )}
            ...
            {(() => {
              ...
              return (
                <aside className="lg:sticky lg:top-28 flex flex-col gap-6">
                  {/* Roadmap Widget */}
                  <RoadmapWidget tasks={visibleTasks} getKLTime={getKLTime} />
        ...
        <div className="h-full">
          ...
          <main className="pt-[140px] lg:pt-28 pb-32 lg:ml-64 px-6 animate-in fade-in duration-300">
            <div className="max-w-7xl mx-auto">
              ...
              {managerSubView === 'ledger' && <ManagerLedgerView config={meritConfig} setConfig={setMeritConfig} activityLog={activityLog} />}
...
function ManagerDashboardView({ team, achievements, activityLog, setAddAchOpen, onDeleteStaff }: { team: TeamMember[], achievements: Achievement[], activityLog: ActivityLog[], setAddAchOpen: (v: boolean) => void, onDeleteStaff: (id: string, name: string) => void }) {
  ...
  const pointsVelocityScore = Math.min(1, avgPoints / minPointThreshold);
      ...
      <section className="grid grid-cols-1 xl:grid-cols-[1.5fr_1fr] gap-10">
        <div className="space-y-10">
          ...
          <div className="bg-surface-container-lowest p-8 rounded-3xl shadow-sm border border-outline-variant/10">
            ...
            {/* Live Team Productivity Chart */}
            <div className="mb-6 p-5 rounded-2xl bg-surface-container border border-outline-variant/10 shadow-sm">
              <div className="flex justify-between items-center mb-4">
                <p className="text-[10px] uppercase font-bold tracking-widest text-on-surface-variant">Points Distribution</p>
                <div className="flex items-center gap-2">
                  <label className="text-[10px] font-bold text-on-surface-variant">Min Target:</label>
                  <input type="number" className="w-16 border border-outline-variant/20 rounded p-1 text-xs bg-white text-on-surface" value={minPointThreshold} onChange={(e) => setMinPointThreshold(Number(e.target.value) || 0)} />
                </div>
              </div>
              <div className="h-[200px] w-full text-xs">
                {team.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={team.map(m => ({ name: m.name.split(' ')[0], points: m.monthPoints }))} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      />
                      <ReferenceLine y={minPointThreshold} stroke="#e11d48" strokeDasharray="3 3" label={{ position: 'top', value: 'Min Target', fill: '#e11d48', fontSize: 10, fontWeight: 'bold' }} />
                      <Bar dataKey="points" fill="#0ea5e9" radius={[4, 4, 0, 0]} barSize={24} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="h-full flex items-center justify-center text-on-surface-variant opacity-50">No data</div>
                )}
              </div>
            </div>
...
function ManagerLedgerView({ config, setConfig, activityLog }: { config: MeritConfig, setConfig: (c: MeritConfig) => void, activityLog: ActivityLog[] }) {
  const ledgerEntries = activityLog.filter(a => a.type === 'points_earned').slice(0, 50);
  return (
    <>
      <div className="mb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="bg-surface-container-lowest p-8 rounded-3xl border border-outline-variant/10 shadow-sm">
          ...
          <div className="space-y-3">
              {ledgerEntries.length === 0 ? (
                <div className="text-center py-10 rounded-2xl border-2 border-dashed border-outline-variant text-on-surface-variant">
                  <p className="text-xs mt-1 opacity-60 font-bold">No point distributions recorded yet.</p>
                </div>
             ) : ledgerEntries.map(entry => (
               <div key={entry.id} className="flex items-center justify-between p-4 rounded-xl hover:bg-surface-container transition-colors border border-outline-variant/5">
                 <div className="flex items-center gap-3">
                   <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                     <span className="material-symbols-outlined text-[20px]">add_circle</span>
                   </div>
                   <div>
                     <p className="text-sm font-bold">{entry.staffName}</p>
                     <p className="text-[10px] text-on-surface-variant uppercase font-bold tracking-widest underline underline-offset-4 decoration-primary/30">{entry.desc}</p>
                   </div>
                 </div>
                 <div className="text-right">
                   <p className="font-extrabold text-primary font-headline">+{entry.points} <span className="text-[8px]">PTS</span></p>
                 </div>
               </div>
             ))}
          </div>
...
// Roadmap Components
// â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• â• 
function RoadmapWidget({ tasks, getKLTime }: { tasks: Task[], getKLTime: () => string }) {
  const klToday = getKLTime().split('T')[0];
  
  // Future Items: not completed, commencement > today
  const futureTasks = tasks.filter(t => t.status !== 'completed' && t.commencementDate && t.commencementDate.split('T')[0] > klToday)
...
function CompletedTasksModal({ isOpen, onClose, tasks }: { isOpen: boolean, onClose: () => void, tasks: Task[] }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200 p-4">
      <div className="bg-surface w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden border border-outline-variant/20 animate-in zoom-in-95 font-sans relative flex flex-col max-h-full">
        <div className="p-6 border-b border-outline-variant/10 flex justify-between items-center bg-surface-container-lowest shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase font-bold tracking-widest text-on-surface-variant opacity-70">Task History</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          ...
          {sorted.map(task => (
            <div key={task.id} className="p-4 rounded-2xl border border-outline-variant/10 bg-surface-container-low flex justify-between items-start">
               <div className="flex-1">
                 <p className="font-bold text-on-surface">{task.title}</p>
                 <p className="text-xs text-on-surface-variant mt-1">{task.note}</p>
                 <p className="text-[10px] font-black uppercase tracking-widest text-primary mt-2 flex items-center gap-1">
                    <span className="material-symbols-outlined text-[13px]">check_circle</span>
                    Completed {task.lastCompletedDate ? new Date(task.lastCompletedDate).toLocaleDateString() : ''}
                 </p>
               </div>
               <div className="bg-primary hover:bg-primary/90 text-white rounded-xl px-3 py-1 text-center shadow-md shadow-primary/20 transition-all font-black text-sm ml-4">
